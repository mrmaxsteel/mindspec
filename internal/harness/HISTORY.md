...
| 2026-06-18 | **PASS** | - | 8-10 | ~110s | **3-hr turn-count investigation (Sonnet 10/10, Haiku 8, 3/3 PASS).** Baseline failed at maxTurns(20)/18t for TWO post-March reasons, neither bd_close: (1) `mindspec instruct` at the sandbox ROOT reports "No Active Work"/idle (the agent boots there), so it spelunked with `bd list` to rediscover the claimed bead; (2) `mindspec complete` blocked by the unconditional ADR-divergence gate (`core.go` `unowned`→`uncovered`), yielding 3× blind retries → max turns. Fixes (deterministically root-caused via a repro harness): `StartDir` into the bead worktree (real implement sessions run there → instruct shows Implementation Mode) + the full ADR coverage triple (`writeSandboxDomainCoverage` ownership + Accepted ADR-0001 + plan `adr_citations`) committed to the BASE before `setupWorktrees` so the spec/bead refs inherit it. Turns 18→8-10; `complete` now exits 0. Separately, `assertMindspecMode("plan")` became UNREACHABLE: under ADR-0023 a minted-but-not-all-closed epic derives `implement` (verified: even with zero ready beads, `state show`=implement). It passed in 2026-03 (pre-derivation-change), so corrected to `assertMindspecMode("implement")`. NOTE: whether that derivation change ("plan when only blocked beads remain" → always implement) is intended or a regression is an open product question. |
