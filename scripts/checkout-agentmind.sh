...
#
# Also writes a `go.work` file at the mindspec module root that pins
# the agentmind sibling via an absolute path. The go.work file lets
# `go build` resolve the wire package even when the worktree depth
# makes `../agentmind` ambiguous (panel-mandated revision: the bead
# worktree's `../agentmind` and the spec worktree's `../agentmind`
# resolve to different parents). The generated go.work is gitignored.
#
...
#        already-present) and go.work has been written. On success the
...
#   MINDSPEC_NO_GOWORK=1 scripts/checkout-agentmind.sh
#     # skip go.work generation (advanced users only)
#
# CI invariant:
#   Set MINDSPEC_REQUIRE_SIBLING=1 to make the companion Go test
#   (internal/specgate/checkout_agentmind_test.go) fail rather than
#   skip when the sibling is absent. The mindspec CI workflow sets
#   this so a missing sibling becomes a hard failure.
...
On success, also writes a gitignored go.work file at the mindspec
module root pinning the sibling via an absolute path so `go build`
resolves the wire package regardless of worktree depth.
...
Environment:
  ...
  MINDSPEC_NO_GOWORK  Set to 1 to skip generating go.work (advanced).
  MINDSPEC_REQUIRE_SIBLING
                      Read by the companion Go test (specgate). Set to
                      1 in CI to make a missing sibling a hard failure
...
# write_goworkfile materializes a go.work file at the mindspec module
# root that pins the agentmind sibling via the resolved absolute path.
# This makes `go build` succeed regardless of how deeply the mindspec
# checkout is nested (the bead worktree's `../agentmind` and the spec
# worktree's `../agentmind` resolve to different parents, but go.work
# uses absolute paths so both worktrees converge on the same sibling).
#
# The generated go.work is gitignored. Skip when MINDSPEC_NO_GOWORK=1
# (e.g. CI that wants to test the bare-replace-directive path).
write_goworkfile() {
    local sibling_abs="$1"
    if [ "${MINDSPEC_NO_GOWORK:-0}" = "1" ]; then
        return 0
    fi
    local gowork="$REPO_ROOT/go.work"
    cat > "$gowork" <<EOF
// AUTO-GENERATED by scripts/checkout-agentmind.sh — do not edit by hand.
//
// This file pins the agentmind sibling for the local worktree so that
// "go build" resolves the wire package regardless of worktree depth.
// The file is gitignored; regenerate by re-running checkout-agentmind.sh.
//
// Remove this file (or rerun with MINDSPEC_NO_GOWORK=1) to fall back to
// the go.mod replace directive's relative path.
