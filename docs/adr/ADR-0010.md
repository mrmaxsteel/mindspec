...
Claude Code reads its OTLP endpoint from settings. The recording system writes to `.claude/settings.local.json` (project-local, gitignored):
...
This is set on recording start and removed on recording stop. If the user already has an OTLP endpoint configured (e.g., pointing to an external collector or a live AgentMind), the recording system must either:
- **Fan-out**: proxy events to both the recording file and the existing endpoint
- **Defer**: skip auto-configuration and warn the user

Fan-out adds complexity. For v1, we defer: if an OTLP endpoint is already configured, warn and don't override. The user can manually point both at the same collector.
