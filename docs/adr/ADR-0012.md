...
2. **Feature reimplementation creates drift.** `propagate.go` reimplements parent-child status propagation that beads molecules handle natively. `next/beads.go` reimplements molecule-aware work discovery that `bd ready --parent` already provides. `plan.go` manually constructs molecules via loops of `Create()` + `DepAdd()` calls when `bd pour` does this in a single command.
