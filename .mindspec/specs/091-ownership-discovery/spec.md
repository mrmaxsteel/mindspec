...
gate consults `.mindspec/domains/<domain>/OWNERSHIP.yaml` to
...
- **ADR record** (`.mindspec/adr/`, not a code domain): A
...
- [ADR-0036-ownership-discovery.md](../../adr/ADR-0036-ownership-discovery.md)
  ...
  **Creation of the ADR file is deferred to the first
  ...
  to author the ADR re-checks `.mindspec/adr/` at
...
8. **`mindspec doctor --fix` writes an empty stub `OWNERSHIP.yaml`**
   ...
   `.mindspec/domains/<domain>/` that lacks one. The stub
   content is:
...
9. **`mindspec domain add <name>` scaffolds an empty-stub
   ...
   parent `.mindspec/domains/` directory but no domain
   subdirectories;
...
10. **A new `mindspec ownership populate <domain>` subcommand
   ...
   ```
   Populate .mindspec/domains/<domain>/OWNERSHIP.yaml
...
- `.mindspec/adr/ADR-0036-ownership-discovery.md` (new).
- `.mindspec/adr/ADR-0031-doc-sync-gate.md` — add the
...
- [ ] **`mindspec doctor --fix` scaffolds an empty-stub
  ...
  `.mindspec/domains/<domain>/` that lacks one. After
...
- [ ] **`mindspec domain add <name>` scaffolds an empty-stub
  ...
  `.mindspec/domains/foo/OWNERSHIP.yaml` matching the
...
- [ ] **The ownership-discovery ADR exists** under
  `.mindspec/adr/` (as `ADR-0036-ownership-discovery.md`
...
- **Missing manifest fires the rewritten Requirement 21 Warn —
  ...
  cd "$tmp"
  mindspec init
  mkdir -p .mindspec/domains/ghost
...
- **`mindspec ownership populate <domain>` prints a prompt:**
  ...
  cd "$tmp"
  mindspec init
  mkdir -p .mindspec/domains/auth
  printf 'paths: []\n' > .mindspec/domains/auth/OWNERSHIP.yaml
...
- **Hand-authored manifest is preserved:**
  ...
  cd "$tmp"
  mindspec init
  mkdir -p .mindspec/domains/foo
  printf 'paths:\n  - cmd/foo-cli/**\n' > .mindspec/domains/foo/OWNERSHIP.yaml
  # shasum -a 256 is portable (stock macOS has no sha256sum):
  before=$(shasum -a 256 .mindspec/domains/foo/OWNERSHIP.yaml)
  mindspec doctor --fix
  after=$(shasum -a 256 .mindspec/domains/foo/OWNERSHIP.yaml)
...
- **`mindspec domain add` scaffolds the empty stub + prints prompt:**
  ...
  test -s .mindspec/domains/foo/OWNERSHIP.yaml \
    || { echo "domain add did not scaffold OWNERSHIP.yaml"; exit 1; }
  grep -qE '^paths:\s*\[\s*\]\s*$' .mindspec/domains/foo/OWNERSHIP.yaml \
    || { echo "expected paths: [] stub"; exit 1; }
  grep -qE '^\s*-\s+internal/foo/\*\*' .mindspec/domains/foo/OWNERSHIP.yaml \
...
- **The `dead-manifest` Warn fires at static `doctor` time
  ...
  mkdir -p .mindspec/domains/ghost
  printf 'paths:\n  - internal/ghost/**\n' \
    > .mindspec/domains/ghost/OWNERSHIP.yaml
...
- **The hygiene Warns fire on a hand-crafted bad manifest:**
  ...
  cd "$tmp"
  mindspec init
  mkdir -p .mindspec/domains/{a,b}
  # Duplicate + subpath redundancy in domain a:
  cat > .mindspec/domains/a/OWNERSHIP.yaml <<'YAML'
paths:
  - internal/a/**
  - internal/a/**
  - internal/a/sub/**
...
YAML
  ...
  cat > .mindspec/domains/b/OWNERSHIP.yaml <<'YAML'
paths:
  - internal/b/**
  - internal/shared/**
YAML
...
- **ADR-0036 present, ADR-0031 amended:**
  ```bash
  test -f .mindspec/adr/ADR-0036-ownership-discovery.md
...
- **`mindspec init` does NOT scaffold domain directories** — by
  ...
  creates the empty parent `.mindspec/domains/` directory;
