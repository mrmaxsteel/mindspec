...
- **ADR-0030** (new — "Executor as the Git/Process I/O Boundary for
  ...
  the file at `.mindspec/adr/ADR-0030-executor-boundary.md`
...
- **ADR number reservation.** At plan-draft time the highest existing
  ...
  free. Bead 4 re-checks `.mindspec/adr/` at PR-open time and
...
8. Finalize `.mindspec/adr/ADR-0030-executor-boundary.md`.
   ...
   - Confirms the frontmatter `Status: Accepted` is unchanged
     ...
     .mindspec/adr/ADR-0030-executor-boundary.md` returns
     line 4).
...
10. **ADR-renumber check** (spec lines 126-134). Run
    `ls .mindspec/adr/ADR-*-executor-boundary.md`. If exactly
...
- [ ] `ls .mindspec/adr/ADR-*-executor-boundary.md | wc -l` returns exactly `1`.
...
- [ ] Spec AC "exactly one ADR file matching `.mindspec/adr/ADR-*-executor-boundary.md` exists" is satisfied.
