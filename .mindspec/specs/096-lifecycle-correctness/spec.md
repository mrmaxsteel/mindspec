...
- **mindspec-8lzq (P2) — VERIFIED OPEN.** `mindspec adr create` writes to the MAIN checkout,
  ...
  into `<main>/.mindspec/adr/`. The OverlayStore added since
...
`.mindspec/domains/workflow/OWNERSHIP.yaml`. Workflow is already covered by the Accepted
...
3. **(bn3u) `adr.NextID` parses the leading integer of slugged ADR filenames.**
   ...
   (`.mindspec/domains/workflow/OWNERSHIP.yaml`) so the changed `internal/adr/parse.go`
...
4. **(8lzq) `adr create` writes into the invoking worktree, not the main checkout.**
   ...
   `<that-worktree>/.mindspec/adr/ADR-NNNN-*.md`, and the file MUST NOT appear in the main
...
- `.mindspec/domains/workflow/OWNERSHIP.yaml` — claim `internal/adr/**` under the workflow
...
- [ ] `mindspec adr create` run from a bead/spec worktree writes the ADR into THAT worktree's
      `.mindspec/adr/`, and the file does NOT appear in the main checkout. RED on revert:
