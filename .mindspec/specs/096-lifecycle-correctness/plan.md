...
> `.mindspec/domains/workflow/OWNERSHIP.yaml` EXCEPT `internal/adr/**`, which Bead 3
...
2. ON the bn3u bead branch, claim `internal/adr/**` in
   `.mindspec/domains/workflow/OWNERSHIP.yaml` (add to the `paths:` list) so the changed
...
1. `cmd/mindspec/adr.go` `adrCreateCmd` RunE (`:28`): replace `workspace.FindRoot(cwd)` with
   ...
   `<worktree>/.mindspec/adr/ADR-NNNN-*.md` and does NOT appear in main. The READ/list
...
3. Tests (RED-on-revert): `adr create` invoked from a synthetic linked worktree writes the ADR
   into THAT worktree's `.mindspec/adr/` and the file does NOT appear in the main checkout's
...
- [ ] `adr create` from a worktree writes into that worktree's `.mindspec/adr/`, not main —
...
- [ ] `mindspec adr create` run from a bead/spec worktree writes the ADR into THAT worktree's
      `.mindspec/adr/`, and the file does NOT appear in the main checkout.
