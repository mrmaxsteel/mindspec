...
> **DESIGN AMENDMENT (operator decision, 2026-07-10) — the DISCHARGE path was DELETED.**
> As-shipped (bead commit `33a8ce8d`), Bead 2 does NOT implement the "discharge-by-re-tally / `refutation_discharged`" mechanism described below (steps 5(c) discharge disjunct, `dischargeEvidence`/`dischargeVerified`, origin binding, the two-disjunct verified-discharge test). During bead review, three successive panel rounds (codex, 0.99 each) each found a distinct FALSE-DISCHARGE hole: reconciliation re-read **mutable** panel files to "verify" a refuted finding had resolved naturally, and an operator could delete/corrupt/alias panel files to fake that (wrong-panel → origin-unbound → basename-cross-root-alias). Rather than keep patching the identity precision of a fundamentally spoofable check, the operator (Max) decided to **remove discharge entirely** and replace it with a **self-contained marker**: the `refutation_pending` entry now carries the full `{slot, round, reason, evidence}`, and reconciliation ALWAYS settles a pending obligation by writing the honest `panel_refuted` **from the marker** (on every path incl. hatches, even after `panel.json` removal) — no re-tally, no panel read at reconcile, no `refutation_discharged` audit. Natural resolution (a re-panel flip with no refutation applied) creates no marker, so it needs no discharge. This eliminates the entire attack class by construction and yields strictly more honest provenance (an applied refutation is always recorded as `panel_refuted` with its reason/evidence). The authoritative description of the shipped design is `reviews/spec-114-bead2/consolidated-round-3.md`; ADR-0037 is amended accordingly in Bead 3. Everything ELSE in this section (schema, single-homed gate validation, `AppliedRefutations`, `panelGate` plumbing, applied≡persisted marker, reconcile-before-close on every path, already-covered no-op, fail-closed `GetMetadata`/`MergeMetadata`, the anti-gaming `internal/panel` layer) shipped AS WRITTEN and is unaffected. The verified 8/8-unanimous round-4 panel (incl. codex) confirmed the redesign. **This amendment is SPEC-WIDE, not section-local:** it supersedes every `discharge` / `refutation_discharged` / verified-discharge / reconciliation-re-tally reference ANYWHERE in this plan (incl. the § above at "satisfying (`panel_refuted`), VERIFIED-discharging …", the Bead-3 instructions that mention writing a `refutation_discharged` audit, ADR Fitness, and Testing Strategy) — as shipped there is no discharge path; reconciliation always satisfies from the self-contained marker.
...
| AC2 (refutation unblocks + audit persists) | Bead 2 | `TestPanelGateDecision_Refutations` (Allow row) + `TestPanelRefuted_WriteMetadata` (shipped name; the plan draft's `TestWritePanelRefutedMetadata` label) + the e2e passing-by-refutation run; `go test ./internal/complete -run 'PanelRefuted\|PanelGate' && go test ./internal/panel -run 'Refut'` |
...
| AC11 (durable-obligation: applied≡persisted+unioned; TOTAL reconciliation; **satisfy-from-self-contained-marker — DISCHARGE DELETED, see the Bead-2 DESIGN AMENDMENT**; every path incl. hatches; fail-closed) | Bead 2 | **As-shipped (`33a8ce8d`), no discharge:** `TestPanelRefuted_SatisfyWriteFailure_FailsCompletion` + `TestPanelRefuted_MarkerWriteFailure_Blocks` (marker-write/union-read-fail ⟹ RC BLOCKS) + `TestPanelRefuted_SatisfyFromMarker_PanelGone` (honest `panel_refuted` written from the self-contained marker even with `panel.json` removed — the case that used to false-discharge) + `TestPanelRefuted_MarkerCarriesReasonEvidence` + `TestPanelRefuted_NaturalResolution_NoObligation` (no refutation applied ⟹ no marker ⟹ zero-ceremony) + `TestPanelRefuted_CrossRun_UnionReconcilesAll` + `TestPanelRefuted_HatchStillReconcilesPendingObligation` (satisfies from marker on hatch paths) + `TestPanelRefuted_GetMetadataError_Refuses` + `TestPanelRefuted_PristineNoPanel_FailsOpen` + `TestPanelRefuted_AlreadyCoveredPending_Completes` + `TestPanelRefuted_MalformedPendingEntries_Refuses` (incl. shape-invalid) + `TestPanelRefuted_AuditSurvivesLaterReadError` + `TestMergeMetadata_FailClosedOnReadError` + `TestGetMetadata`; `go test ./internal/complete -run 'PanelRefuted\|PanelGate' && go test ./internal/bead -run 'MergeMetadata\|GetMetadata'`. *(The deleted discharge tests — `..._NaturalResolution_Discharges`, `..._WarnPathDoesNotDischarge`, `..._CrossPanel_NoFalseDischarge`, `..._OriginRemoved/Corrupted_Refuses`, `..._CrossRun_PanelRemoved_Refuses` — defended the removed re-tally mechanism and are gone by construction.)* |
