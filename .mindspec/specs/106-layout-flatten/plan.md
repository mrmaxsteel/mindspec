...
- **Bead 6 (post-move cleanup — skills + governance + rubric + fixtures):**
  ...
  `project-docs/` + the flat children never `project-docs/user/` (AC24).
...
1. Introduce a relative-path layout-classifier predicate and make EVERY git-ref
   ...
   `.mindspec/specs/<id>/` literals in `validateSpecArtifactSync`/`specMDID`
...
5. Make `internal/contextpack/budgeter.go` consume the Bead-1 per-artifact accessors
   instead of the hardcoded `.mindspec/specs/<id>` (budgeter.go:170) and
   `.mindspec/domains/<d>/<kind>` (budgeter.go:218) joins, so a context pack
...
`.mindspec/domains/workflow/OWNERSHIP.yaml` (claim the package); with golden
fixtures + `*_test.go`.
...
4. Build the FINITE-PATTERN markdown link-rewriter — NOT a general parser. Rewrite
   ...
   `.mindspec/core/`/context-map refs, review-co-location depth changes, dogfood
...
2. **MIGRATE the existing root `review/**` tree (R6 blocker 5 — currently missing).**
   ...
   `.mindspec/specs/099-panel-gate-in-complete/reviews/099-final-panel/`, `…/prep`, loose `*.md`). Run the mover's
...
3. Update each domain's `OWNERSHIP.yaml` that self-claims
   `.mindspec/domains/<d>/OWNERSHIP.yaml` to the flat
...
- [ ] No domain `OWNERSHIP.yaml` self-claims the pre-flatten `.mindspec/domains/` path (AC20)
...
1. Update EVERY path-bearing LIVE skill/setup-text surface to the post-flatten paths
   ...
   (`.mindspec/domains/`) AND next-ADR-number scan (`.mindspec/adr/`) — its
...
3. Reconcile the `cmd/mindspec/migrate.go` prompt rubric (migrate.go:264-298,
   ...
   never to `project-docs/user/`/`…/agent/`.
...
5. Verify: `go test ./internal/setup/...` is green; the AC17 breadth grep over the FULL
   ...
   the flat children, never `project-docs/user/`.
...
- [ ] `go test ./cmd/mindspec/...` asserts the migrate rubric targets `project-docs/` + the flat children, never `project-docs/user/` (AC24)
