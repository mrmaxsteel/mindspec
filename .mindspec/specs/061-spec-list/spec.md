...
There is currently no command to list specs. Users must manually `ls .mindspec/specs/` or use `mindspec state show` (which only shows the active spec). With ADR-0023 eliminating focus files, lifecycle phase is now derived from beads — a list command can show each spec's phase by querying beads epics.
...
1. `mindspec spec list` prints a table of all specs found under `.mindspec/specs/`
...
- `mindspec spec list --json | jq length`: Returns count matching `ls .mindspec/specs/ | wc -l`
