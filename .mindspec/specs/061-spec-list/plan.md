...
1. Create `internal/speclist/speclist.go`:
   ...
   - `List(root string) ([]SpecEntry, error)`: scan `.mindspec/specs/`, read each `spec.md` frontmatter for status, call `phase.FindEpicBySpecID()` + `phase.DerivePhase()` for phase, sort by spec number
