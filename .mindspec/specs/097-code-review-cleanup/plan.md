...
The ownership prerequisite. Edit `.mindspec/domains/core/OWNERSHIP.yaml` to add BOTH
...
**Steps**
1. Edit `.mindspec/domains/core/OWNERSHIP.yaml`: append `internal/idvalidate/**` and
...
**Verification**
- [ ] `.mindspec/domains/core/OWNERSHIP.yaml` lists both `internal/idvalidate/**` and
...
**Acceptance Criteria**
- [ ] `.mindspec/domains/core/OWNERSHIP.yaml` claims both `internal/idvalidate/**` and
...
1. Add a `WorkChunk` type (`id int`, `depends_on []int`) and a `work_chunks []WorkChunk` field to
   ...
   `{id, depends_on}`: the user template (`project-docs/user/templates/plan.md:15-27`) and the
...
6. Update BOTH plan templates to emit/teach the structured `work_chunks` form with integer ids and
   ...
   prose-only) and `project-docs/user/templates/plan.md` (reconcile its letter-suffixed headings
...
4. Emit `key_file_paths` PER WORK-CHUNK in BOTH plan templates, nested under each `work_chunks[*]`
   entry (`internal/instruct/templates/plan.md` and `project-docs/user/templates/plan.md`).
