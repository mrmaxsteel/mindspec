...
`.mindspec/domains/*/OWNERSHIP.yaml` coverage and leaves the fixture plan's `adr_citations`
...
1. In `ScenarioImplApprove`'s `Setup` (`internal/harness/scenario_spec_lifecycle.go` ~370-415), BEFORE
   ...
   `.mindspec/domains/sandbox/OWNERSHIP.yaml` (claiming `done.go`) and the Accepted
   `.mindspec/adr/ADR-0001-sandbox-domain.md` are present on the base `main` ref the
