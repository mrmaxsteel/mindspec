...
- **ADR-0034** (new, stub at
  `.mindspec/adr/ADR-0034-ceremony-collapse.md`, `Status:
...
- **Bead 3** (`internal/doctor/dry_run_migration_test.go` — new
  ...
  - `TestDoctorDryRunMigrationReports` — fixture with two specs
    under a `t.TempDir()`-rooted `.mindspec/specs/`: one
...
| `TestDoctorDryRunMigrationReports` | Bead 3 | Direct test; verifies the dry-run reporter walks `.mindspec/specs/`, identifies legacy epics, prints `would-migrate` lines, and writes nothing. |
...
implement the reporter that walks `.mindspec/specs/`, and
...
- **MODIFIED** `internal/doctor/migration.go` — add
  ...
  `.mindspec/specs/`, identifies legacy epics lacking
...
- **MODIFIED** `.mindspec/adr/ADR-0034-ceremony-collapse.md`
...
5. **ADR-0034 finalization.** In
   `.mindspec/adr/ADR-0034-ceremony-collapse.md`, replace
...
- [ ] `TestDoctorDryRunMigrationReports` passes; reporter
  walks `.mindspec/specs/`, identifies legacy epics, emits
...
doctor walks .mindspec/specs/, identifies legacy 7-bead specs
