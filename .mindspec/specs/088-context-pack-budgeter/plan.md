...
- **ADR-0033** (new — "Pluggable Tokenizer Interface and Deterministic
  ...
  `.mindspec/adr/ADR-0033-tokenizer-interface.md` already carries
...
4. Update
   `.mindspec/domains/context-system/OWNERSHIP.yaml` to ADD
...
- [ ] `git diff
  .mindspec/domains/context-system/OWNERSHIP.yaml` — shows
...
5. Implement `BuildBead` body. The algorithm (matching spec
   ...
   7. Resolve domain docs: for each impacted domain (sorted),
      attempt `os.ReadFile(".mindspec/domains/<domain>/overview.md")`
...
7. Add new tests to `internal/contextpack/budgeter_test.go` per
   ...
   skeleton (`.mindspec/specs/<id>/spec.md`, `plan.md`,
   `.mindspec/adr/ADR-XXXX-*.md`,
   `.mindspec/domains/<name>/overview.md`,
   `.mindspec/domains/<name>/interfaces.md`,
   `.mindspec/domains/<name>/OWNERSHIP.yaml`) from string
...
4. Finalize ADR-0033. Edit
   `.mindspec/adr/ADR-0033-tokenizer-interface.md` lines
...
- [ ] `git diff
  .mindspec/adr/ADR-0033-tokenizer-interface.md` — shows
