...
- **mindspec-vvs9 (LIVE):** the diff range is now ref-anchored, but OWNERSHIP *attribution*
  ...
  `os.ReadFile(filepath.Join(root, ".mindspec/domains/<domain>/OWNERSHIP.yaml"))` — it
...
- [ADR-0031](../../adr/ADR-0031-doc-sync-gate.md): doc-sync OWNERSHIP attribution — Req 1 changes the tree
  the manifest is read from (ambient working tree → diffed ref).
- [ADR-0036](../../adr/ADR-0036-ownership-discovery.md): OWNERSHIP loader (spec 091 Req 13, no silent fallback) —
