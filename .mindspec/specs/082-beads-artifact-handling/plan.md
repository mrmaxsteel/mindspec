...
**Steps**
1. Draft `.mindspec/adr/ADR-0025-jsonl-as-build-artifact.md` with Status=Accepted. Content: Dolt is authoritative per ADR-0023; `.beads/issues.jsonl` is a deterministic projection co-managed by `bd export` and mindspec's commit points; tooling must not treat JSONL diffs as user authorship.
...
5. Update `.mindspec/domains/workflow/overview.md` (or the closest matching workflow-domain doc if that exact path doesn't exist) with a short "JSONL as artifact" section referencing ADR-0025 by number.
...
- [ ] ADR-0025 exists at `.mindspec/adr/ADR-0025-jsonl-as-build-artifact.md` with Status=Accepted, cross-references ADR-0023 in its body, and is already present in this plan's `adr_citations` frontmatter (no plan edit required).
