...
Close the gaps identified in the happy-path review so that the end-to-end MindSpec development workflow — from `/spec-init` through `mindspec complete` — works without the agent needing undocumented tribal knowledge or manual workarounds. Then strip back the static instruction files (`CLAUDE.md`, `AGENTS.md`, `.claude/rules/*`, `.claude/commands/*`) to minimal bootstraps, delegating all mode-specific guidance to `mindspec instruct`.
