...
under `.mindspec/docs/domains/<name>/`. As of spec 086, the seeded
set is exactly four:
...
creating `.mindspec/docs/domains/<name>/OWNERSHIP.yaml`; once that
...
- **ADR number reservation.** At spec-draft time the highest
  ...
  `.mindspec/docs/adr/` at PR-open time; if `ADR-0032` has been
...
9. **Divergence check at complete (and unowned-file detection).**
   ...
   - **(b) Unowned file (revision 4).** When `attributeDomain`
     ...
     .mindspec/docs/domains/<name>/OWNERSHIP.yaml", file,
     impactedDomains))`.
...
- [ ] `TestSupersedeUnblocks` passes (revision 1, 9): the bead
  ...
  `.mindspec/docs/adr/ADR-0099-*.md` with parseable frontmatter
...
- [ ] `ADR-0032-adr-semantic-gates.md` exists under
  `.mindspec/docs/adr/`, status `Accepted`, citing ADR-0030 and
