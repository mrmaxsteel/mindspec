...
`.mindspec/docs/domains/<name>/`) to include in the context bundle.
...
`.mindspec/docs/specs/<spec>/plan.md`, and beads are addressed by
...
The existing `.mindspec/docs/domains/context-system/OWNERSHIP.yaml`
...
- **ADR number reservation.** At spec-draft time the highest
  ...
  `.mindspec/docs/adr/` at PR-open time; if `ADR-0033` has been
...
8. **`BuildBead(beadID string, maxTokens int, tok
   ...
   - The spec file at `.mindspec/docs/specs/<spec>/spec.md`,
   ...
   - The plan file at `.mindspec/docs/specs/<spec>/plan.md`,
   ...
   - Each cited ADR's `## Decision` section, loaded via
     ...
     rooted at `.mindspec/docs/adr/`.
   - For each impacted domain in `spec.Domains`: `overview.md` and
     `interfaces.md` under `.mindspec/docs/domains/<domain>/`.
...
12. **`OWNERSHIP.yaml` update.** As part of this spec,
    `.mindspec/docs/domains/context-system/OWNERSHIP.yaml` is
...
- [ ] `TestContextPackErrorOnMissingSpecID` passes: a bead JSON
  ...
  `filepath.Walk` is invoked under `.mindspec/docs/specs/` —
...
- [ ] `ADR-0033-tokenizer-interface.md` exists under
  `.mindspec/docs/adr/`, has `Status: Accepted`, lists
