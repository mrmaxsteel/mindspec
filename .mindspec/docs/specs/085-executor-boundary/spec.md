...
- No prior ADR speaks to the executor abstraction directly. A survey
  of `.mindspec/docs/adr/` (ADR-0001 through ADR-0029) for
...
- **ADR number reservation.** At consensus time the highest existing
  ...
  re-check `.mindspec/docs/adr/` at PR-open time; if `ADR-0030` has
...
- [ ] Exactly one ADR file matching
  `.mindspec/docs/adr/ADR-*-executor-boundary.md` exists, named
...
- **ADR-0030 (or next free integer) present, and exactly one such ADR:**
  ```
  ls .mindspec/docs/adr/ADR-*-executor-boundary.md
  ...
  test "$(ls .mindspec/docs/adr/ADR-*-executor-boundary.md | wc -l)" -eq 1
  ```
