...
- core: `internal/spec/**` (Requirement 5) is already claimed by the core
  ...
  * Requirement 0 edits `.mindspec/docs/domains/core/OWNERSHIP.yaml` to claim
...
> `.mindspec/docs/user/templates/plan.md` is a `.mindspec/docs/**` process
...
0. **Ownership prerequisite — claim the two unowned packages FIRST (gate
   ...
   *What must be true after:* a single doc-only bead edits
   `.mindspec/docs/domains/core/OWNERSHIP.yaml` to add BOTH
...
3. **Declare and consume bead dependencies in structured plan frontmatter
   ...
   *What must be true after (all atomic, one cutover — no window where both prose
   ...
   - BOTH plan templates are updated to emit/teach the structured `work_chunks`
     ...
     `.mindspec/docs/user/templates/plan.md` (currently has `work_chunks` but
...
- **(a) B0 ownership bead is FIRST.** B0 (Requirement 0) edits
  `.mindspec/docs/domains/core/OWNERSHIP.yaml` to add `internal/idvalidate/**`
...
### In Scope
- `.mindspec/docs/domains/core/OWNERSHIP.yaml` — Requirement 0 (claim
...
- `.mindspec/docs/user/templates/plan.md` — Requirements 3, 4 (user template:
...
- [ ] R0: `.mindspec/docs/domains/core/OWNERSHIP.yaml` claims both
