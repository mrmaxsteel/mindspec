...
`.mindspec/docs/domains/<domain>/OWNERSHIP.yaml`; when absent, the
...
- **`internal/validate/`** — `docsync.go` is the centerpiece. The
  ...
  accompanied by `plan.md` or `.mindspec/docs/specs/<id>/` updates.
  ...
  `.mindspec/docs/domains/<domain>/OWNERSHIP.yaml` if present, else
...
- **NEW: `.mindspec/docs/domains/<domain>/OWNERSHIP.yaml`** —
...
- [ADR-0031-doc-sync-gate.md](../../adr/ADR-0031-doc-sync-gate.md)
  ...
  set (`CLAUDE.md`, `CONVENTIONS.md`, `.mindspec/docs/user/**`,
  `.mindspec/docs/core/USAGE.md`), and the `--allow-doc-skew`
...
8. **`validateSpecArtifactSync` lane.** New function in `docsync.go`:
   if any changed file matches `.mindspec/docs/specs/<id>/spec.md`,
   require a sibling change to `plan.md` or any other file under
   `.mindspec/docs/specs/<id>/`. Emits an `AddError` on failure.
   ADR-only diffs under `.mindspec/docs/specs/<id>/` count as
   satisfying the lane.
9. **Per-domain `OWNERSHIP.yaml` is honored.** New helper
   ...
   ```
   It reads `.mindspec/docs/domains/<domain>/OWNERSHIP.yaml` when
...
10. **Operator-docs lane (warning, additive accept set).** `cmd/`
    ...
    under `.mindspec/docs/user/`, or `.mindspec/docs/core/USAGE.md`.
...
15. **`mindspec doctor` warns** when any domain directory under
    `.mindspec/docs/domains/` lacks `OWNERSHIP.yaml`. Warning only;
...
16. **OWNERSHIP.yaml created for every existing domain.** As part of
    ...
    `.mindspec/docs/domains/`. The four known-existing domains and
...
- Operator-docs lane (warning-only, additive accept set per
  ...
  `CONVENTIONS.md`, `.mindspec/docs/user/**`, or
...
- Authoring `OWNERSHIP.yaml` for every existing domain directory
  under `.mindspec/docs/domains/` (`context-system`, `core`,
  `execution`, `workflow`).
...
- [ ] `TestCompleteBlocksOnDocSkew` passes: a bead whose diff
  ...
  `.mindspec/docs/domains/context-system/OWNERSHIP.yaml` or the
...
- [ ] `TestApproveImplBlocksOnSpecDocSkew` passes: a spec branch
  ...
  `.mindspec/docs/specs/086-doc-sync/spec.md` without touching
...
- [ ] `TestOperatorDocsAdditiveAcceptSet` passes: a diff touching
  ...
  file under `.mindspec/docs/user/`, or `.mindspec/docs/core/USAGE.md`
...
- [ ] An `OWNERSHIP.yaml` exists for every currently-existing
  domain directory under `.mindspec/docs/domains/` in this repo
