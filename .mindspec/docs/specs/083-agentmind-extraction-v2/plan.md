...
- **ADR-0011** (One-way `mindspec → agentmind` dependency via OTLP/HTTP:4318): this
  ...
  3a and 3b) and every deletion (Bead 7) tightens compliance with the one-way
...
- **ADR-0026** (to be authored by Bead 9 — "AgentMind extracted to standalone
  repo"): captures what shipped, the deferred decisions (UI-port discovery,
  version-skew, `setup` ownership, first-party installer), and the rollback
  procedure (`git revert <merge-sha>` + drop the `require` line). Drafted
  alongside the Phase 5 deletion (Bead 7) so the ADR text mirrors the
  as-shipped state. Not in this plan's frontmatter `adr_citations` because the
  ADR doesn't exist at plan-approval time; Bead 9 amends the frontmatter as
  part of its commit.
...
4. **Read-side consumer rewire to NDJSON-over-stdout** (Bead 3b): replaces
   ...
   is the load-bearing read-side wire that makes Bead 7's deletion of the
   OTLP parser per-commit-green by construction.
5. **Live-capture CI gate** (Bead 5, spec Test D Phase-4+ variant): once the
   ...
   published in `agentmind/wire` (spec lines 251-255).
6. **Static boundary checks in CI from Phase 4 onward** (Bead 7 makes them
...
- **Test C** (per-class degradation): Bead 3a (telemetry-as-output + batch,
  including `mindspec agentmind setup`) + Bead 4 (interactive).
- **Test D** (live capture): Phase-2 byte-for-byte variant in Bead 2;
  Phase-4+ normalization-and-diff variant wired into CI by Bead 5.
- **Test E** (no-circular-discovery): green after Bead 7's deletions;
  the grep is run in CI from Bead 5 onward. Post-deletion continuity is
  preserved by re-running the grep against a CI-only shallow clone of the
  agentmind sibling repo at the pinned tag (see Bead 8).
- **Test F** (import-boundary): green after Bead 7's deletions; checked
  in CI from Bead 5 onward.
...
1. **Precondition: agentmind v0.1.0 tag exists.** Run
   ...
   (Test-G-style gate, analogous to Bead 1 step 1). Capture the SHA. If the
   tag does not exist yet, this bead is blocked.
2. Confirm the Phase 1 method-set precondition from the spec: run
   ...
   - **Branch A (zero methods found, OR methods already moved to
     `agentmind/wire` in Phase 1):** the alias strategy proceeds as drafted.
   - **Branch B (methods found and not movable to `agentmind/wire`):** fall
     ...
     327-333). In this branch, this bead replaces the local types with
     full duplicates in mindspec (matching `wire.CollectedEvent` field-for-
     field) and schedules deletion of the bench-side copies for Bead 7
     (Phase 5). The alias-vs-duplicate decision MUST be recorded in the
     bead's commit message.
3. Edit `go.mod`: add `require github.com/mrmaxsteel/agentmind v0.1.0`
   (the Phase 1 tag, confirmed in step 1) and `replace
   github.com/mrmaxsteel/agentmind => ../agentmind`.
4. Edit `internal/bench/collector.go`: under Branch A, replace the local type
   ...
   parsing code stays in place for now (deleted in Bead 7).
5. Add `scripts/checkout-agentmind.sh`: clones agentmind at the tag pinned in
   `go.mod` to a sibling directory (`../agentmind`) so CI can resolve the
   `replace` directive. Hook into the CI workflow at the existing
   pre-test step.
6. **Generate the parity fixture via the canonical encoder.** Before the alias
   swap (or, under Branch B, before the duplicate substitution), use the
   canonical `wire/event.go` encoder (from the sibling agentmind checkout at
   v0.1.0) to emit a representative event set under a frozen clock. The
   encoder MUST enforce lex-sorted JSON keys, `strconv.FormatFloat(f, 'f', -1,
   64)` for floats, and UTC `RFC3339Nano` timestamps (spec lines 247-250).
   Commit the resulting fixture at `internal/bench/testdata/parity.ndjson`
   in a **separate prior commit** with provenance recorded in the commit
   message (encoder version, fixture generator command). Ad-hoc dumps from
   the pre-edit OTLP-parsing code are NOT acceptable unless step 6a (below)
   verifies byte-for-byte equality with the canonical encoder.
7. (Optional, only if pre-edit dumps are reused) **Canonical-equivalence
   ...
   assertion forces regeneration via step 6.
8. Add the Phase-2 NDJSON parity test as
...
- [ ] Commit history shows the fixture provenance commit (step 6) preceding
      the alias-swap commit (step 4).
...
1. **Precondition: agentmind v0.3.0 tag exists.** Run
   ...
   (Test-G-style gate, analogous to Bead 1 step 1). Capture the SHA. If the
   tag does not exist yet, this bead is blocked.
2. Bump `go.mod` require to `agentmind v0.3.0` (the Phase 4 tag exposing
   `client.AutoStart` and `client.ErrBinaryNotFound`).
3. Edit `internal/recording/collector.go`: replace the
   ...
   error returns.
4. Edit `internal/bench/runner.go`: same swap. On
...
5. Edit `cmd/mindspec/setup.go` (or wherever `mindspec agentmind setup` is
   ...
   no-rewire conclusion) and produce a setup-specific integration test
   (step 7 below).
6. Add a unit test asserting that the `sync.Once` warn-line emission from
   `agentmind/client` produces exactly one line across multiple `AutoStart`
   calls in the same process. Test pattern: invoke `AutoStart` three times
   with the binary absent; assert stderr contains exactly one occurrence of
   `WARN: agentmind binary not found; telemetry export will drop silently`.
7. Add per-class integration tests covering spec Test C:
...
8. **Typed-sentinel prohibition assertion.** Add a unit-test assertion that
...
Bead 7's deletion of the OTLP-parsing code per-commit-green by construction.
...
5. **OTLP-parser-residue verification.** Add a grep-style assertion that no
   ...
   OTLP-parsing code is the one deleted by Bead 7 step 3 (and which is
...
## Bead 4: Phase 4b — cobra re-exec wrappers for `mindspec viz`, `agentmind serve`, `agentmind replay`

`cmd/mindspec/viz.go`'s subcommands AND the top-level `mindspec viz` command
become thin shells around `client.RunStandalone(args)`. Closes the
interactive-class side of spec Test C and locks in the `--help` parity
contract.

**Steps**
1. Edit `cmd/mindspec/viz.go`: replace the `agentmindServeCmd` and
   `agentmindReplayCmd` `RunE` bodies with a single call to
   `client.RunStandalone(os.Args[1:])` (or the equivalent helper exported
   from `agentmind/client` at v0.3.0). On
   `errors.Is(err, client.ErrBinaryNotFound)`, exit non-zero per the
   interactive class.
2. **Wire `mindspec viz` to the subprocess.** The top-level `vizCmd` is the
   user-facing entry point per spec Hard Constraint #5 (`mindspec viz` is
   the alias for `mindspec agentmind`). Either (a) declare `vizCmd` as a
   cobra alias of `agentmindServeCmd` (one rewire covers both — record this
   decision in the commit message), OR (b) rewrite `vizCmd`'s `RunE` to
   call `client.AutoStart` + `client.RunStandalone(os.Args[1:])` with the
   same `ErrBinaryNotFound` handling as step 1. Verify the user-visible
   behavior matches: with the binary absent, `mindspec viz` exits non-zero
   (interactive class).
3. **Capture the pre-Bead-2 `--help` golden fixture.** Capture
   ...
   **strictly before** Bead 2's alias swap (so the golden cannot
   ...
   source commit SHA in this bead's commit-message metadata. Store the
   golden output at `cmd/mindspec/testdata/help_golden/{serve,replay,viz}.txt`.
4. Add a unit test that the cobra flag definitions for `serve`, `replay`,
   and `viz` are unchanged from pre-extraction by asserting byte equality
   between current `--help` output and the golden captured in step 3. This
   covers the spec AC "`mindspec agentmind serve --help` and `mindspec
   agentmind replay --help` print the same usage text as before the
   extraction."
5. Add an integration test exercising the interactive class of spec Test C:
   `mindspec viz`, `mindspec agentmind serve`, `mindspec agentmind replay`
   each exit non-zero with the binary absent.

**Verification**
- [ ] `go build ./cmd/mindspec && go test -short ./...` passes.
- [ ] `--help` golden test passes (spec AC: same usage text) for `serve`,
      `replay`, and `viz`. The golden's source commit SHA is recorded in
      this bead's commit message and is verifiably pre-Bead-2.
- [ ] Spec Test C interactive-class assertions pass for `viz`,
      `agentmind serve`, and `agentmind replay`.
- [ ] Manual smoke: `./bin/mindspec agentmind serve --help` prints the
      same text as the pre-extraction reference output.

**Depends on**
Beads 3a, 3b.

## Bead 5: Phase 4c — wire Test D (live capture) into mindspec CI

Activates the Phase 4+ semantic-equivalence variant of spec Test D, plus
the static Tests E and F checks, in CI from Phase 4 onward. The
live-capture assertion uses the **spec-mandated normalization tool**
published in `agentmind/wire`, not a substring check.

**Steps**
1. **Precondition: agentmind normalization tool is shipped.** Run a
   precondition gate analogous to Bead 1 step 1
   (`git ls-remote --tags` style) confirming that the agentmind tag in
   `go.mod` (v0.3.0 or later) publishes the normalization tool/library
   under `github.com/mrmaxsteel/agentmind/wire` (spec lines 251-255). If
   the normalizer is not yet shipped at the time this bead runs, this bead
   is blocked. Capture the agentmind SHA in the commit message.
2. Add a CI job (or extend the existing matrix) that:
   - Runs `scripts/checkout-agentmind.sh` to materialize a sibling agentmind
     checkout at the pinned tag.
   - Builds the agentmind binary from the sibling checkout (`go build -o
     ../agentmind/bin/agentmind ./cmd/agentmind`).
   - Runs `./agentmind/bin/agentmind serve --otlp-port 4318 --ui-port 0
     --output /tmp/em.ndjson &`.
   - POSTs a synthetic OTLP log payload via `curl`.
   - Runs the agentmind/wire normalization tool against
     `/tmp/em.ndjson` (sort events by event time, redact PID/host fields,
     redact wall-clock timestamps to canonical placeholder per spec lines
     251-255).
   - Diffs the normalized output against a reference fixture committed at
     `internal/bench/testdata/live_capture_reference.ndjson` (itself
     produced via the canonical encoder, analogous to Bead 2's parity
     fixture). The diff MUST be clean.
3. Add CI steps for spec Tests E and F (cheap greps):
   - **Test E:** `grep -rEn 'exec\\.Command.*"mindspec"|LookPath.*"mindspec"|StartProcess.*"mindspec"' ../agentmind/client/ ../agentmind/cmd/ ../agentmind/internal/`
     ...
     form is NOT.
   - **Test F:** `go list -deps ./cmd/mindspec | grep mrmaxsteel/agentmind |
     sort -u | grep -vE '^github.com/mrmaxsteel/agentmind/(client|wire)$'`
     returns no match.
4. Gate the live-capture job on Bead 1's Test G result so the job is skipped
   (not failed) if the agentmind v0.0.1 tag isn't reachable — defensive
   against upstream unavailability.
...
- [ ] Normalization-tool precondition gate (step 1) passes.

**Depends on**
Beads 3a, 3b, 4.

## Bead 6: (reserved — was original Bead 5; rolled into Bead 5 above)

*(No content. This number is reserved to avoid collisions with downstream
references; the original Bead 5 content was Bead 5 above. Renumbering of
subsequent beads continues at Bead 7.)*

## Bead 7: Phase 5 — deletion of `internal/agentmind/`, `internal/viz/`, and OTLP-parsing code; binary-size delta
...
7. **Author ADR-0026 in this same bead** so the ADR mirrors the as-shipped
   state — see Bead 9 for content; the commit that deletes the code is the
   right commit to add the ADR alongside. Cross-reference both ways:
   ADR-0026 cites the deletion commit SHA; this plan's `adr_citations`
   frontmatter gets ADR-0026 appended.
...
- [ ] Spec Tests E and F pass by absence within mindspec (Bead 8 handles
...
## Bead 8: Phase 6 — drop `replace`, pin `agentmind v1.0.0`, document manual install, preserve Test E continuity
...
preserves Test E continuity post-Bead-7 by re-running the grep against a
...
3. Update the mindspec CI live-capture job (Bead 5) to download the released
...
4. **Preserve Test E continuity** (spec Test E, no-circular-discovery).
   Once Bead 7 deletes `internal/agentmind/`, the in-mindspec grep that
   ...
   this step is the mindspec-side mirror. (Alternative considered and
   rejected by the panel: declaring Test E green-by-construction
   post-deletion; the panel recommended the CI-shallow-clone mirror as the
   stronger gate.)
5. **Soak gate (replaces spec line 373's 7-day soak).** Convert the
   ...
   - Maintain a nightly-CI artifact `agentmind-phase3-soak-history.txt`
     listing each date the Phase 3 integration test (in the agentmind
     repo) ran green.
   - Before merging this bead, assert seven consecutive green entries
     ending at a date no earlier than `today - 1 day`. The CI gate fails
     if the green-streak is broken or the latest entry is stale.
   - Link to the nightly run history in the commit message.
   - **Deferral path:** if the nightly soak artifact is not yet wired (as
     it depends on agentmind-side CI), record the deferral explicitly in
     the commit message and gate merging this bead on a documented
     alternative (e.g. one-shot manual integration test re-run on
     mindspec's PR branch before merge). Either form MUST be verifiable;
     "7 days" alone is not.
...
- [ ] CI live-capture job pulls the released binary, verifies its checksum,
      and runs Test D successfully (normalization-and-diff per Bead 5).
...
## Bead 9: Author ADR-0026 and finalize cross-references

ADR-0026 is *drafted* alongside the Bead 7 deletion so its text mirrors what
shipped; this bead is the **finalization** — once Beads 7 and 8 are merged,
the ADR's "as-shipped" content is locked, cross-references are added, and
the plan's `adr_citations` frontmatter is updated.

**Steps**
1. Finalize `.mindspec/docs/adr/ADR-0026-agentmind-extracted-to-standalone-repo.md`
   with Status=Accepted. Required sections:
   - **Context:** ADR-0011 established the one-way dep; this ADR captures
     the physical separation.
   - **Decision:** AgentMind lives at `github.com/mrmaxsteel/agentmind`;
     mindspec imports only `client` and `wire`; IPC is OTLP/HTTP inbound +
     NDJSON-over-stdout outbound.
   - **Deferred decisions:** UI-port discovery (port 8420 stays hardcoded),
     version-skew handling (no `client.Probe()` checks for v1.0.0),
     `mindspec agentmind setup` ownership (stays in mindspec for now),
     first-party `mindspec install agentmind` subcommand (deferred to
     follow-up spec).
   - **Rollback procedure:** `git revert <merge-sha>` of the mindspec PR +
     drop the `require github.com/mrmaxsteel/agentmind` line from `go.mod`.
   - **Cross-reference ADR-0011** in the body.
2. Edit this plan's frontmatter (`adr_citations`) to add `ADR-0026`.
3. Edit `spec.md`'s ADR Touchpoints section to replace the
   "ADR-0026 (new, to be authored as part of this work)" placeholder with a
   live link to the finalized ADR file.
4. Run `mindspec doctor` (or equivalent ADR-existence validator) and confirm
   it reports ADR-0026 as present.

**Verification**
- [ ] `bd show ADR-0026` (or filename existence) succeeds.
- [ ] Plan frontmatter `adr_citations` includes `ADR-0026`.
...
|---|---|
| `find internal/agentmind` returns no results | Bead 7 verification |
| `grep -r 'http.HandleFunc.*"/v1/logs"' .` returns no results | Bead 7 verification |
| `go list -deps ./cmd/mindspec` shows only `client` + `wire` from agentmind (Test F) | Bead 5 CI step + Bead 7 (green by absence) |
...
| `mindspec agentmind setup` exits 0 with exactly one centralized warn line (batch) — OR documented as satisfied-by-design if `setup` does not call `AutoStart` | Bead 3a step 5 + Bead 3a integration test (setup-specific) |
| NDJSON byte-for-byte parity (Phase 2, alias state) — fixture produced via canonical `wire/event.go` encoder | Bead 2 step 6 (canonical-encoder fixture) + Bead 2 parity test |
| NDJSON semantic equivalence (Phase 4+, subprocess state) via spec-mandated normalization tool | Bead 5 CI live-capture job (normalize-and-diff against reference fixture) |
...
| mindspec binary size shrinks; before/after recorded | Bead 7 commit message |
| ADR-0026 committed and referenced from spec's ADR Touchpoints | Bead 9 (drafted in Bead 7, finalized here) |
| Release branch has `require agentmind v1.0.0` and no `replace` directive | Bead 8 verification |
...
| Test D (live capture) — Phase 4+ normalize-and-diff | Bead 5 |
| Test E (no-circular-discovery) — pre-deletion | Bead 5 CI step against sibling agentmind checkout |
| Test E (no-circular-discovery) — post-deletion continuity | Bead 8 CI shallow-clone step against pinned agentmind v1.0.0 |
| Test F (import-boundary) | Bead 5 CI step + Bead 7 (green by absence within mindspec) |
...
| Test-G-style precondition gate, agentmind/wire normalization tool published | Bead 5 step 1 |
