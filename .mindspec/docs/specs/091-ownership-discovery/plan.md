...
- **ADR-0014** (canonical document root under `.mindspec` —
  ...
  `OWNERSHIP.yaml` files under `.mindspec/docs/domains/<domain>/`
...
- **Proposed new ADR — ownership discovery (ZFC)**: Bead 1 authors
  ...
  **`ADR-0036-ownership-discovery.md`, or the next free integer
  found by re-checking `.mindspec/docs/adr/` on BOTH the current
...
file under `.mindspec/docs/adr/`, ADR-0031 amendment, `spec.md`
...
2. Claim the ADR number: re-check `.mindspec/docs/adr/` on this
...
- [ ] The new ADR file exists with a non-0035 number;
      `grep -c "ADR-0035" .mindspec/docs/specs/091-ownership-discovery/spec.md`
