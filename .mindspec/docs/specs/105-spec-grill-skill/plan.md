...
- **ADR-0036 (Ownership Discovery)** — Accepted; Domain(s) **workflow,
  ...
  `.mindspec/docs/domains/*/OWNERSHIP.yaml` using exactly this model, and the
...
2. Encode the **grill protocol (R1):** instruct the agent to ask exactly ONE
   ...
   against the directories under `.mindspec/docs/domains/`, surface the real
   ADRs under `.mindspec/docs/adr/` relevant to each impacted domain, suggest
...
`internal/setup/claude_test.go`, `.mindspec/docs/domains/workflow/OWNERSHIP.yaml`
...
4. **Ownership claim (R12 / AC11):** add `- .agents/skills/**` to
   `.mindspec/docs/domains/workflow/OWNERSHIP.yaml` so the git-tracked
...
- [ ] `grep -qE '^\s*- \.agents/skills/\*\*' .mindspec/docs/domains/workflow/OWNERSHIP.yaml` PASS and `! grep -qE 'bench/' .mindspec/docs/domains/workflow/OWNERSHIP.yaml` PASS (AC11)
