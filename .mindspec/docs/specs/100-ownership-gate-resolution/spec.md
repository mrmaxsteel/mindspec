...
turns that name into `.mindspec/docs/domains/<name>/OWNERSHIP.yaml` — a directory
lookup. So when a spec's Impacted Domains are FILE PATHS (e.g.
`genevieve/review.py`), the gate looks for
`.mindspec/docs/domains/genevieve/review.py/OWNERSHIP.yaml`, finds nothing, and
...
- workflow: every changed source file lands here. Under normalize-at-source
  ...
  ADR-0032 amendment is a doc under `.mindspec/docs/adr/**`, which the divergence
...
1. **(bead mindspec-4ft2) Normalize Impacted-Domains entries to owning domain
   ...
   - an entry that already names a domain directory (a domain dir whose
     `.mindspec/docs/domains/<entry>/OWNERSHIP.yaml` exists) is KEPT verbatim;
...
3. **(bead mindspec-3cfr) `adr show` / `adr list` are worktree-aware; lock the
   ...
   **Acceptance Criteria**
   - [ ] A hermetic test exercising the `adr show` (and `adr list`) root-resolution
     ...
     `.mindspec/docs/adr/` is found and rendered with its `Domain(s)`, where the
...
- `.mindspec/docs/adr/ADR-0032-adr-semantic-gates.md` — amendment note: path-like
