...
**Prompt commands**: Use `/spec-init`, `/spec-approve`, etc. in the chat panel — these are defined in `.github/prompts/` and instruct Copilot to run the corresponding `mindspec` CLI command.
...
Or use `/spec-init` in Copilot Chat.
...
Or use `/spec-approve` in Copilot Chat.
...
Or use `/plan-approve` in Copilot Chat.
...
Or use `/impl-approve` in Copilot Chat.
