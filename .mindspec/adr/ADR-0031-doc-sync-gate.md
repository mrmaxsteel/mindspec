...
> `.mindspec/domains/widget/OWNERSHIP.yaml` claiming
...
`.mindspec/domains/<domain>/OWNERSHIP.yaml` for path→domain
...
1. **Warnings → Errors at the doc-sync call sites; operator-docs lane
   ...
   `project-docs/user/` touches) deliberately REMAINS `AddWarning`,
...
2. **Per-domain `OWNERSHIP.yaml`, co-located.**
   `.mindspec/domains/<domain>/OWNERSHIP.yaml` with schema
