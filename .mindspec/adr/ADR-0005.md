...
> **Update (2026-02-26, Spec 046):** This section has been revisited. `state.json` is now **gitignored**. ADR-0015 demoted it from "primary source of truth" to a non-canonical convenience cursor — mode is derived from molecules, not state.json. Spec 046 added machine-local fields (`activeWorktree` with absolute paths, `specBranch`) that are meaningless across machines. Session recovery uses `mindspec instruct` which queries the molecule directly. The original rationale for committing (session continuity, project bookmark) is now served by molecules instead.
